...
function jumpPage(pageUrl, jq_container) {
	// /
	var param = {
		paramurl : basePath + 'sdey/jumpPage',
		paramdata : {
			pageUrl : pageUrl
		},
		dataType : 'html',
...
function layerWindowFrame(pageUrl, title, width, height) {
			jumpPageLayerNum = layer.open({
				type : 2,
				title : title,
				skin : 'layui-layer-rim', // 加上边框
...
function initTable() {
	...
	$("a[ajaxFrame]").click(function() {
		var href = $(this).attr("href");
		var title = $(this).attr("title");
		var width = $(this).attr("width");
		var height = $(this).attr("height");
		layerWindowFrame(href, title, width, height);
		return false;
	})
...
// /////让form ajax提交
function initAjaxFormForSearch(jq_form,jq_container,formURL,runBT) {
	var zzcid = null;
	function showResponse(data) {
		layer.close(zzcid);
		jq_container.empty().append(data);
	}
	function showRequest() {
		var validate = true;
		$("input[required]").each(function(){
			...
			if($.trim(ival).length==0){
				var jq_span=$("<span style='color:red'></span>").text("必填项！").attr("id","err-"+$(this).attr("id"));
				$(this).click(function(){
					var removeJQ=$("#err-"+$(this).attr("id"));
					removeJQ.remove();
				});
				$(this).after(jq_span);
				validate=false;
